...
| `Mods/ModSystem.cs`, `Integration/GorillaTagPlayer.cs` | Movement mods for your own player: Platforms (hold grip / C), Fly (hold X / F), Speed Boost, Low Gravity, Big Monke, and a checkpoint you can save and return to. A lobby gate only allows them offline, in private rooms and in modded (Utilla `MODDED_`) rooms, and switches everything off when you join a public lobby. Nothing targets other players. |
