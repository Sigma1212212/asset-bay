...
| `Mods/ModSystem.cs`, `Mods/MoreMods.cs`, `Integration/GorillaTagPlayer.cs` | Mods for your own player. Movement: Platforms (hold grip / C), Fly (hold X / F, adjustable speed), Noclip, Speed Boost (adjustable), Low Gravity, Hover, Dash (B / V), Rewind (left trigger / R), Size (adjustable), Checkpoint. Visual: ESP (boxes, names, distance, tracers through walls), Hand Trails, Speedometer, Freecam (desktop). Plus a Grapple gun mode. A lobby gate only allows them offline, in private rooms and in modded (Utilla `MODDED_`) rooms, and switches everything off in public lobbies. Nothing changes other players' games. |
